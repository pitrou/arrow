...
struct PivotImpl : public ScalarAggregator {
  Status Init(const PivotOptions& options, const std::vector<TypeHolder>& in_types) {
    options_ = &options;
...
Result<std::unique_ptr<KernelState>> PivotInit(KernelContext* ctx,
  ...
  const auto& options = checked_cast<const PivotOptions&>(*args.options);
...
const FunctionDoc pivot_doc{
    ...
    ("Output is a struct with as many fields as `PivotOptions.key_names`.\n"
...
void RegisterScalarAggregatePivot(FunctionRegistry* registry) {
  static auto default_pivot_options = PivotOptions::Defaults();

  auto func = std::make_shared<ScalarAggregateFunction>(
      "pivot", Arity::Binary(), pivot_doc, &default_pivot_options);
