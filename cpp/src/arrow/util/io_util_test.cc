...
// It's a bit annoying to test for invalid fds on Windows as they may trigger
// assertion errors crashes in debug mode.
...
class TestFileDescriptor : public ::testing::Test {
 public:
  ...
  void AssertValidFileDescriptor(int fd) {
    ASSERT_OK_AND_ASSIGN(bool is_valid, IsValidFileDescriptor(fd));
    ASSERT_TRUE(is_valid) << "Not a valid file descriptor: " << fd;
  }

  void AssertInvalidFileDescriptor(int fd) {
    ASSERT_OK_AND_ASSIGN(bool is_valid, IsValidFileDescriptor(fd));
    ASSERT_FALSE(is_valid) << "Unexpectedly valid file descriptor: " << fd;
  }
